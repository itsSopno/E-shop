...
const KeycapDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const items = data.keycaps || data;
        const found = items.find((item: Keycap) => item._id === id);
        setKeycap(found);
      } catch (error) {
        console.error("Error fetching keycap detail:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#080808] flex items-center justify-center">
      <div className="w-12 h-12 border-2 border-[#D9FF00]/20 border-t-[#D9FF00] rounded-full animate-spin"></div>
    </div>
  );
  ...
  return (
    <main className="relative min-h-screen bg-[#080808] text-white selection:bg-[#D9FF00] selection:text-black">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto min-h-screen flex flex-col lg:flex-row items-center justify-center p-6 lg:p-12 gap-12 lg:gap-24">
        ...
        {/* --- LEFT: PRODUCT DISPLAY (No JS Animation) --- */}
        <div className="w-full lg:w-1/2 flex items-center justify-center group">
          <div className="relative w-full aspect-square max-w-[550px] bg-[#111] rounded-3xl border border-white/5 overflow-hidden flex items-center justify-center p-12 transition-transform duration-500 hover:scale-[1.02]">
            ...
            <div className="absolute bottom-6 left-6 bg-black/60 backdrop-blur-md px-3 py-1 rounded border border-white/10">
              <p className="text-[10px] font-mono tracking-tighter text-white/40">SERIAL: 0x{id?.slice(-6).toUpperCase()}</p>
            </div>
          </div>
        </div>
